...
export const FirebaseProvider: FC<FirebaseProviderProps> = ({
  ...
  const handleSignInWithPlatform = async (
    ...
    try {
      ...
      // 默认错误处理
      console.log(`[${projectName} Firebase] Error during sign-in`, error);
      const errorCode = (error as any).code;
      const errorMessage = (error as any).message;
      // 用户账号使用的邮箱
      const email = (error as any).customData?.email;
      // 这里可以根据错误做进一步处理，比如显示错误消息给用户
      console.log(`[${projectName} Firebase] Error during sign-in with: ${platform}`);
      console.log(`[${projectName} Firebase] Error code: ${errorCode}`);
      console.log(`[${projectName} Firebase] Error message: ${errorMessage}`);
      console.log(`[${projectName} Firebase] User email: ${email}`);
      throw error;
    }
  };
  ...
  const signUpWithEmailAndPassword = async (email: any, password: any) => {
    try {
      ...
      console.log(`[${projectName} Firebase] Verification email sent.`);
  ...
  const handleSignOut = () => {
    signOut(auth)
      .then(() => {
        ...
        // Sign-out successful.
        console.log(`[${projectName} Firebase] Sign out successful`);
      })
      .catch((error: any) => {
        // An error happened.
        console.log(`[${projectName} Firebase] Sign out error`, error);
        throw error;
      });
  };
  ...
  const updateUserProfile = async (userInfo: Partial<User>) => {
    ...
    try {
      ...
      // Profile updated!
      console.log(`[${projectName} Firebase] User profile updated:`, userInfo);
    } catch (error) {
      // An error occurred
      console.log(`[${projectName} Firebase] Error updating user profile:`, error);
      throw error;
    }
  };
  ...
  const updateCurrentUserPassword = async (newPassword: string) => {
    ...
    if (currentUser) {
      try {
        ...
        // 更新成功
        console.log(`[${projectName} Firebase] Password update successful.`);
      } catch (error) {
        // 错误处理
      ...
      console.log(`[${projectName} Firebase] No user is currently signed in.`);
    }
  };
